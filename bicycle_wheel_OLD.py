...
class BicycleWheel:
    ...
    def calc_k_spoke(self, s, tension=0.0):
    ...
    def calc_continuum_stiff(self, tension=0.0):
        ...
        return len(self.spokes) / (4*np.pi*self.rim.radius) * \
            (k_spoke(s1) + k_spoke(s2))

    def calc_Pn_lat(self):
        'Lateral Pippard number (ratio of length scale to spoke spacing).'

        k_sp = self.calc_continuum_stiff()
        k_uu = k_sp[0, 0]

        n_spokes = len(self.spokes)
        EI = self.rim.young_mod * self.rim.I22
        GJ = self.rim.shear_mod * self.rim.I11
        cc = (GJ/EI)/(GJ/EI + 1)

        Pn_lat = n_spokes/(2*np.pi*self.rim.radius) *\
            np.power(4*EI / k_uu, 0.25) * cc

        return Pn_lat

    def calc_Pn_rad(self):
        ...
        n_spokes = len(self.spokes)
        EI = self.rim.young_mod * self.rim.I33

        Pn_rad = n_spokes/(2*np.pi*self.rim.radius) *\
            np.power(4*EI / k_vv, 0.25)

        return Pn_rad

    def calc_lambda_lat(self):
        'Calculate lambda = k_uu*R^4/EI_lat'

        k_sp = self.calc_continuum_stiff()
        k_uu = k_sp[0, 0]

        return k_uu*self.rim.radius**4 / (self.rim.young_mod * self.rim.I22)

    def calc_lambda_rad(self):
        'Calculate lambda = k_vv*R^4/EI_rad'

        k_sp = self.calc_continuum_stiff()
        k_vv = k_sp[1, 1]

        return k_vv*self.rim.radius**4 / (self.rim.young_mod * self.rim.I33)

    def print_stats(self):
        'Print summary information about the wheel.'

        print 'lambda (lat) :', self.calc_lambda_lat()
        print 'lambda (rad) :', self.calc_lambda_rad()
        print 'R/le (lat)   :', np.power(self.calc_lambda_lat(), 0.25)
        print 'R/le (rad)   :', np.power(self.calc_lambda_rad(), 0.25)
        print 'Pn_lat       :', self.calc_Pn_lat()
        print 'Pn_rad       :', self.calc_Pn_rad()
